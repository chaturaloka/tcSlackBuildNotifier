        ...
        function populateSlackNotificationDialog(id) {
            ...
            jQuerySlacknotification.each(ProjectBuilds.projectSlacknotificationConfig.slackNotificationList, function (thing, config) {
                if (id === config[0]) {
                    ...
                    jQuerySlacknotification('#funnyQuoteCharacter').val(slacknotification.funnyQuoteCharacter)
                }
            });
            updateSelectedBuildTypes();
        }
        ...
        function addSlackNotificationsFromJsonCallback() {
            jQuerySlacknotification.each(ProjectBuilds.projectSlacknotificationConfig.slackNotificationList, function (thing, config) {
                if ('new' !== config[0]) {
                    ...
                    jQuerySlacknotification("#viewRow_" + slacknotification.uniqueKey + " > td.slackNotificationRowItemChannel").html(slacknotification.channel).click(function () {
                        BS.EditSlackNotificationDialog.showDialog(slacknotification.uniqueKey, '#hookPane');
                    });
                    jQuerySlacknotification("#viewRow_" + slacknotification.uniqueKey + " > td.slackNotificationRowItemEvents").html(slacknotification.enabledEventsListForWeb).click(function () {
                        BS.EditSlackNotificationDialog.showDialog(slacknotification.uniqueKey, '#hookPane');
                    });
                    jQuerySlacknotification("#viewRow_" + slacknotification.uniqueKey + " > td.slackNotificationRowItemBuilds").html(slacknotification.enabledBuildsListForWeb).click(function () {
                        BS.EditSlackNotificationDialog.showDialog(slacknotification.uniqueKey, '#buildPane');
                    });
                    jQuerySlacknotification("#viewRow_" + slacknotification.uniqueKey + " > td.slackNotificationRowItemEdit > a").click(function () {
                        BS.EditSlackNotificationDialog.showDialog(slacknotification.uniqueKey, '#hookPane');
                    });
                    jQuerySlacknotification("#viewRow_" + slacknotification.uniqueKey + " > td.slackNotificationRowItemDelete > a").click(function () {
                        BS.SlackNotificationForm.removeSlackNotification(slacknotification.uniqueKey, '#hookPane');
                    });
    ...
    <div class="editBuildPageGeneral" style="background-color:white; float:left; margin:0; padding:0; width:70%;">
        ...
        <c:choose>  
    		<c:when test="${haveBuild}"> 
			    ...
			    To edit all slacknotifications for builds in the project <a
                    href="index.html?projectId=${projectExternalId}">edit Project slacknotifications</a>.
         	</c:when>  
         	<c:otherwise>  
        ...
        <div class=configurationSection>
            ...
            <c:choose>
				<c:when test="${ShowFurtherReading == 'ALL'}">
				          <p>Further Reading:
				          <ul>${moreInfoText}
                              <li><a href="https://github.com/PeteGoo/tcSlackBuildNotifier">tcSlackBuildNotifier
                                  plugin</a></li>
                          </ul>
				</c:when>
